...
module.exports = {
    run: async (message, args, client) => {
        ...
        if(member.id === message.guild.ownerID) return message.channel.send("Vous ne pouvez pas warn un administrateur !")
        ...
        message.channel.send(`**${member.user.tag.substring(0, member.user.tag.length-5)}** a été warn pour **${reason}** !`)
    },
    name : "warn",
    help: {
        description: "Permet de mettre un avertissement à un membre",
        systaxe: '<@membre> <raison>'
    },
    guildOnly : true
}